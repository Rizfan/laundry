...
<div class="card shadow mb-4" width="100%">
	<div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
		...
		<a href="/admin/tambah_transaksi/upload/{{$outlet_id}}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-upload fa-sm text-white-50"></i> Tambah Transaksi</a>
	</div>
	<div class="card-body">
		<div class="table-responsive">
			<table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
				...
				<tbody>
					...
					<tr>
						<td>{{$no++}}</td>
						<td><a href="/admin/detail_transaksi/{{$data->id_transaksi}}">{{$data->kode_invoice}}</a></td>
						...
						<td>
							...
							<a class="text-light" href="/admin/invoice/{{$data->id_transaksi}}"><button class="btn btn-primary"><i class="fas fa-print"></i></button></a>
						</td>
					</tr>
...
<div class="modal fade" id="pdfModal" tabindex="-1" aria-labelledby="pdfModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form method="POST" enctype="multipart/form-data" action="/admin/list_transaksi/pdf">
				@csrf
				<div class="modal-body">
					<input type="hidden" name="id_outlet" value="{{$outlet_id}}">
...
<script type="text/javascript">
	...
	$('.hapus').on('click',function(){
		...
		console.log(id)
		var url = '{{URL::to('admin/list_transaksi/hapus_transaksi')}}/' +id
	...
	$('.batal').on('click',function(){
		var id = $(this).attr('acc-id')
		console.log(id)
		var url = '{{URL::to('admin/list_transaksi/batal_bayar')}}/' +id
